...
use tokio::{io, prelude::*, net::UdpSocket};
...
where F: FnMut(&str, u16) -> Result<T, io::Error>
{
    type Socket = T;
...
impl<R, U, MR, MU> SwitchableUdpSocket<R, U, MR, MU>
{
    ...
    pub fn new(iface: &str, port: u16, mode: SocketMode, mut make_raw: MR, mut make_udp: MU) -> Result<SwitchableUdpSocket<R, U, MR, MU>, io::Error> 
    where
        R: UdpAsyncReadWrite,
        U: UdpAsyncReadWrite,
    ...
    MU: MakeSocket<Socket = U>,
{
    fn switch_to(&mut self, mode: SocketMode) -> Result<(), io::Error> {
        ...
        if mode != self.mode() {
            ...
            self.socket = new_socket(&self.iface, self.port, mode, &mut self.make_raw, &mut self.make_udp)?;
        }
        Ok(())
    }
