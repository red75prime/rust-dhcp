...
mod test {
    use super::*;
    use crate::{HardwareType, OperationCode, Options, MessageType};
    use eui48::MacAddress;

    #[test]
    fn dhcp_discover_serialization_simple() {
        let msg = Message {
            operation_code: OperationCode::BootRequest,
            hardware_type: HardwareType::Ethernet,
            hardware_address_length: 6,
            hardware_options: 0,
            transaction_id: 0xd3b5f7d0,
            seconds: 0,
            is_broadcast: false,
            ...
            boot_filename: vec![],
            options: Options {
                dhcp_message_type: Some(MessageType::DhcpDiscover),
                client_id: Some(vec![1, 1, 2, 3, 4, 5, 6]),
                dhcp_max_message_size: Some(1500),
                vendor_class_id: Some(Vec::from(b"android-dhcp-8.0.0" as &[u8])),
                hostname: Some("MI6-MiPhone".into()),
                parameter_list: Some(vec![1, 3, 6, 15, 26, 28, 51, 58, 59, 43]),
                .. Options::default()
            }
        };
        let mut buf = vec![0; 1500];
        let packet_len = msg.to_bytes(&mut buf, None).unwrap();
        let buf = &buf[..packet_len];
        assert_eq!(buf, &vec![
            0x01, 0x01, 0x06, 0x00, 0xd3, 0xb5,
            0xf7, 0xd0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x00, 0x00, 0x00, 0x00,
            ...
            0xdc, 0x37, 0x0a, 0x01, 0x03, 0x06, 0x0f, 0x1a, 0x1c, 0x33, 0x3a, 0x3b, 0x2b, 0x3d, 0x07, 0x01,
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x0c, 0x0b, 0x4d, 0x49, 0x36, 0x2d, 0x4d, 0x69, 0x50, 0x68,
            0x6f, 0x6e, 0x65, 0x3c, 0x12, 0x61, 0x6e, 0x64, 0x72, 0x6f, 0x69, 0x64, 0x2d, 0x64, 0x68, 0x63,
            0x70, 0x2d, 0x38, 0x2e, 0x30, 0x2e, 0x30, 0xff
        ][..]);
    }
}
